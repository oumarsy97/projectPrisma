...
export default class UserController {
  ...
  static updateUser = async (req: Request, res: Response) => {
    ...
    if (!validationResult.success) {
      return res
        .status(400)
        .json({ message: validationResult.error.message, status: 400 });
    }
    try {
  ...
  static getUser = async (req: Request, res: Response) => {
    ...
    if (!idUser) {
      return res
        .status(400)
        .json({ message: "Invalid user ID", data: null, status: 400 });
    }
    try {
      const user = await prisma.user.findUnique({
        where: {
  ...
  static addCredit = async (req: Request, res: Response) => {
    try {
      ...
      if (!user)
        return res
          .status(404)
          .json({ message: "User not found", data: null, status: 400 });

      const mycode = await prisma.generateCode.findFirst({
        where: { code: codeString },
      });
      if (!mycode)
        return res
          .status(404)
          .json({ message: "Code not valid", data: null, status: 404 });
      if (mycode.status === "USED")
        return res
          .status(400)
          .json({ message: "Code already used", data: null, status: 400 });

      const tailor = await prisma.actor.findUnique({
        where: { idUser: parseInt(idUser) },
      });
      if (!tailor)
        return res
          .status(404)
          .json({ message: "Tailor not found", data: null, status: 404 });
  ...
  static achatCode = async (req: Request, res: Response) => {
    try {
      ...
      if (montant < 100)
        return res
          .status(400)
          .json({ message: "Montant invalide", data: null, status: 400 });

      const newCode = await prisma.generateCode.create({
        data: {
          price: montant,
      ...
      if (user.phone) {
        await Messenger.sendSms(
          user.phone,
          "Tailor Digital",
          `Votre code de paiement est : ${recu}`
        );
      }
      if (user.email) {
  ...
  static becomeTailor = async (req: Request, res: Response) => {
    try {
      ...
      Messenger.sendMail(
        user.email,
        "Tailor Digital",
        `Votre compte est maintenant un Tailor vous avez 50 credits`
      );
      Messenger.sendSms(
        user.phone,
        "Tailor Digital",
    ...
    try {
      ...
      if (!user)
        return res
          .status(404)
          .json({ message: "User not found", data: null, status: 404 });
      const vendor = await prisma.actor.findUnique({
        where: { idUser: parseInt(idUser) },
      });
      if (!vendor)
        return res
          .status(404)
          .json({ message: "Vendor not found", data: null, status: 404 });
