...
export default class StoryController {
  static create = async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!actor) {
        return res
          .status(404)
          .json({ message: "Actor not found", data: null, status: false });
      }
  ...
  static deleteStory = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const story = await prisma.story.findFirst({
        where: { id: Number(idStory), idActory: actor.id },
      });
      if (!story) {
        return res
          .status(404)
          .json({ message: "Story not found", data: null, status: false });
      }
      ...
      res
        .status(500)
        .json({ message: (error as Error).message, data: null, status: false });
    }
  };

  static viewStory = async (req: Request, res: Response) => {
    try {
      const { idStory } = req.params;
  ...
  static getStoryViews = async (req: Request, res: Response) => {
    try {
      const { idStory } = req.params;
      const userId = req.params.userId;
  ...
  static getMyStories = async (req: Request, res: Response) => {
    try {
  ...
  static getMyFollowingStories = async (req: Request, res: Response) => {
    try {
      const idUser = req.params.userId;
      if (!idUser) {
        return res
          .status(401)
          .json({ message: "Unauthorized", data: null, status: false });
      }
